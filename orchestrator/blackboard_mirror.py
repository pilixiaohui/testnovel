...
MIRROR_ALLOWED_SUFFIXES = frozenset({".md", ".json"})
...
def _iter_sync_files(root: Path) -> list[Path]:
    files: list[Path] = []
    for path in root.rglob("*"):
        if path.is_file() and path.suffix.lower() in MIRROR_ALLOWED_SUFFIXES:
            files.append(path)
    return sorted(files)


def _compute_blackboard_tree_hash(*, mirror_root: Path) -> tuple[str, int]:
    ...
    for subdir in MIRROR_SUBDIRS:
        ...
        for path in _iter_sync_files(current):
    ...
    if files_count == 0:
        raise RuntimeError(f"No sync files found in mirror root: {mirror_root.as_posix()}")

    return digest.hexdigest(), files_count


def sync_project_blackboard_mirror(
    *,
    ...
    for subdir in MIRROR_SUBDIRS:
        ...
        for source_file in _iter_sync_files(src_dir):
            rel = source_file.relative_to(src_dir)
            target_file = dst_dir / rel
            target_file.parent.mkdir(parents=True, exist_ok=True)
            target_file.write_bytes(source_file.read_bytes())

    content_hash, files_synced = _compute_blackboard_tree_hash(mirror_root=mirror_root)
    ...
    manifest = {
        ...
        "file_suffixes": sorted(MIRROR_ALLOWED_SUFFIXES),
...
def sync_project_markdown_mirror(
    *,
    ...
    """Backward-compatible alias; prefer `sync_project_blackboard_mirror`."""
    return sync_project_blackboard_mirror(
        context=context,
        iteration=iteration,
        triggered_by=triggered_by,
    )
...
def assert_project_mirror_unchanged(*, mirror_root: Path, expected_hash: str, stage: str) -> None:
    current_hash, _ = _compute_blackboard_tree_hash(mirror_root=mirror_root)
