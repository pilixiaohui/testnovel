...
| **状态提取模型** | **Gemini 1.5 Flash** | **书记员**。文本定稿后的后台处理，低成本提取实体状态变更，生成“状态变更预案”供人工审核。 |
| **图数据库** | **Kùzu Embedded** | **叙事关系引擎**。存储人物关系网（雪花法第5步）及动态剧情状态。支持 Cypher 查询语言，进行复杂的多跳逻辑检索。 |
...
### **3.2 通用实体与状态模型 (Entity & State Models)**
  ...
  * **RelationEdge (动态关系)**:
      * 除了基础的 `type`，增加 `narrative_tension` (叙事张力值，0-100)。雪花法第 5 步的人物关系网直接转化为图谱中的边。

### **3.3 数据库映射策略**

  * **Kùzu Graph Schema**:
      * Nodes: `StoryRoot`, `Act`, `Scene`, `Entity`
      * Edges:
          * `(:StoryRoot)-[:CONTAINS]->(:Act)-[:CONTAINS]->(:Scene)` (结构树)
          * `(:Scene)-[:FEATURES]->(:Entity)` (场景涉及人物)
          * `(:Entity)-[:RELATES_TO {tension: 80}]->(:Entity)` (社会关系)
...
1.  **结构检索**：从 Kùzu 读取该场景的 `expected_outcome`（来自雪花大纲）。
2.  **状态检索**：读取当前所有在场 `Entity` 的最新 `semantic_states`。
...
1.  **Gemini 1.5 Flash** 分析生成的文本，提取潜在的实体状态变更（如 `HP: 100% -> 10%`, `Location: Safehouse -> Unknown`）。
...
async def on_scene_complete(scene_id: UUID, content: str, user_interface_callback):
    ...
    # 3. 写入数据库 (仅写入人工确认过的数据)
    await kuzu_db.batch_update_states(verified_changes)
    
    # 4. 后台触发影响分析 (Fire and Forget)
...
async def handle_ripple_effect(scene_id, content):
    ...
    scene = await kuzu_db.get_scene(scene_id)
...
1.  **里程碑 1：雪花引擎 (Week 1-2)**
      * 实现 Logline 到 Scene List 的自动扩写算法。
      * 搭建 React Flow 前端，可视化展示生成的大纲树。
2.  **里程碑 2：图谱状态机与分支系统 (Week 3-4)**
      * 集成 Kùzu，实现 Entity 和 Scene 的存储。
      * 开发 Git 式分支管理逻辑与 `Dirty` 状态传播算法。
3.  **里程碑 3：协商闭环与人机验证 (Week 5-6)**
      * 开发 Gemini 3 Pro 的逻辑推演 Prompt，实现 `ImpactLevel` 的判定与 `Logic Exception` 处理。
      * **重点开发**：前端“状态确认面板”与后端的状态变更 Diff 算法，打通 HITL 流程。
4.  **里程碑 4：全量测试 (Week 7-8)**
      * 导入 10 本经典小说的结构数据进行微调测试。
...
这个 V3.0 方案不仅仅是一个工具，它是一种**全新的创作工作流**。
  ...
  * 它利用**前沿 AI（Gemini 3 + Kùzu）**，保证剧情有逻辑，不会崩坏。
